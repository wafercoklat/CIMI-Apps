                ...
                <div class="card-body">
                    <div class="table-responsive overflow-hidden">
                        ...
                        <table class="table table-hover pb-2" width="100%" id="datatable">
                            ...
                            <tbody>
                                @foreach ($iso as $iso) @if ($iso->stats<> 'On Schedule')
                                    <tr>
                                        <td><a href="#" id="#stats"><span class="badge @if ($iso->stats == 'Ready')bg-success @elseif ($iso->stats == 'In Use' or $iso->stats == 'Suspend' or $iso->stats == 'Void' or $iso->stats == 'In Use - On Schedule')bg-danger @elseif ($iso->stats == 'On Schedule')bg-warning @endif">{{$iso->stats}}</span></a></td>
                                        <td><a href="{{route('isotank.info', $iso->id)}}"><span class="badge bg-secondary text-black">{{$iso->code}}</span></a></td>
                                        <td> @if ($iso->stats
                                            <> 'Ready')<a href="#" type="button" data-target="#modalshow{{$iso->id}}" data-toggle="modal"><span class="badge  bg-info">{{$iso->transnumber}} <i class="mdi mdi-arrow-right"></i></span></a>@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready')@if ($iso->tgl_outdepo != NULL) {{date('d M Y', strtotime($iso->tgl_outdepo))}} @else <span>-</span> @endif @endif</td>
                                        <td>@if ($iso->tgl_indepo != NULL) {{date('d M Y', strtotime($iso->tgl_indepo))}} @else <span>-</span> @endif</td>
                                        <td>@if ($iso->stats == 'Ready'){{$iso->des}} @else {{$iso->ori}} @endif</td>
                                        <td>@if ($iso->stats <> 'Ready'){{$iso->ori}} -> {{$iso->des}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->customername}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->lokasi_loading}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->lokasi_bongkar}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->vessel}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->packinglist_no}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->partai}}@endif</td>
                                    ...
                                    <tr>
                                        <td><a href="#" id="#stats"><span class="badge @if ($iso->stats == 'Ready')bg-success @elseif ($iso->stats == 'In Use' or $iso->stats == 'Suspend' or $iso->stats == 'Void' or $iso->stats == 'In Use - On Schedule')bg-danger @elseif ($iso->stats == 'On Schedule')bg-warning @endif">{{$iso->stats}}</span></a></td>
                                        <td><a href="{{route('isotank.info', $iso->id)}}"><span class="badge bg-secondary text-black">{{$iso->code}}</span></a></td>
                                        <td>@if ($iso->stats <> 'Ready')<a href="#" type="button" data-target="#modalshow{{$iso->id}}" data-toggle="modal"><span class="badge  bg-info">{{$iso->v_transnumber}} <i class="mdi mdi-arrow-right"></i></span></a>@endif</td>
                                        ...
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->v_lokasi_loading}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->v_lokasi_bongkar}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->v_vessel}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->v_packinglist_no}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->v_partai}}@endif</td>
                                        <td>@if ($iso->stats
                                            <> 'Ready'){{$iso->v_createdby}}@endif</td>
                                    </tr>
                                    <div class="modal fade" id="modalshow{{$iso->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="row">
                                                            <div class="form-group col-lg-6 col-md-12 col-sm-12">
                                                                <label class="filter-set">Tanggal Muat / Loading</label>
                                                                <input type="date" @if ($iso->v_tgl_muat
                                                                <> NULL) value="{{\Carbon\Carbon::parse($iso->v_tgl_muat)->format('Y-m-d')}}" @else value="" @endif class="form-control filter-set" aria-label="Tanggal Awal" name="tgl_muat">
                                                            </div>
                                                            <div class="form-group col-lg-6 col-md-12 col-sm-12">
                                                                <label class="filter-set">Tanggal ETD</label>
                                                            ...
                                                            <div class="form-group col-lg-6 col-md-12 col-sm-12">
                                                                <label class="filter-set">Tanggal Bongkar / Dooring</label>
                                                                <input type="date" @if ($iso->v_tgl_bongkar
                                                                <> NULL) value="{{\Carbon\Carbon::parse($iso->v_tgl_bongkar)->format('Y-m-d')}}" @else value="" @endif name="tgl_bongkar" class="form-control filter-set" aria-label="Tanggal Awal">
                                                            </div>
                                                            <div class="form-group col-lg-6 col-md-12 col-sm-12">
                                                                <label class="filter-set">Tanggal InDepo*</label>
                                                                <input type="date" @if ($iso->v_tgl_indepo_real
                                                                <> NULL) value="{{\Carbon\Carbon::parse($iso->v_tgl_indepo_real)->format('Y-m-d')}}" @else value="" @endif name="tgl_indepo_real" class="form-control filter-set">
                                                            </div>
                                                        </div>
                                                        <div class="row parent">
